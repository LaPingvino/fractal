...
mod imp {
    ...
    impl ComposerState {
        ...
        pub(super) fn set_edit_source(&self, message_event: &MessageEventSource) {
            ...
            let (text, formatted) = match message_event.msgtype() {
                Some(MessageType::Emote(emote)) => {
                    (format!("/me {}", emote.body), emote.formatted.clone())
                }
                Some(MessageType::Text(text)) => (text.body.clone(), text.formatted.clone()),
                _ => return,
            };
            ...
            if room.permissions().can_notify_room() && message_event.can_contain_at_room() {
        ...
        match &draft_type {
            ...
            ComposerDraftType::Reply { event_id } | ComposerDraftType::Edit { event_id } => {
                ...
                match draft_type {
                    ...
                    ComposerDraftType::NewMessage => unreachable!(),
                }
            }
        }
    }
